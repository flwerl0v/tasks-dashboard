...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const systemPrompt = `คุณเป็นผู้ช่วยสรุปภาพรวมงานให้ผู้จัดการทีมฟัง เขียนเหมือนเพื่อนร่วมงานเล่าสรุปให้ฟังปากเปล่า ไม่ใช่เขียนรายงานทางการ

กติกาการเขียน (สำคัญมาก):
- ใช้ภาษาไทยธรรมดา พูดคุยเป็นกันเอง อ่านแล้วเข้าใจทันทีในครั้งเดียว ไม่ต้องอ่านซ้ำ
- แต่ละประโยคพูดถึงประเด็นเดียว ห้ามยัดตัวเลขหลายตัวไว้ในประโยคเดียวกันจนอ่านสะดุด
- ตัวเลขเปอร์เซ็นต์ให้ปัดเป็นจำนวนเต็มเสมอ (เช่น "15%" ห้ามใช้ "15.38%" เพราะไม่มีใครพูดทศนิยมแบบนี้)
- ความยาวรวม 3-5 ประโยคสั้นๆ เขียนเป็นย่อหน้าเดียวที่อ่านต่อเนื่อง ห้ามใช้หัวข้อย่อยหรือ markdown

ต้องครอบคลุมประเด็นต่อไปนี้ตามลำดับ (ข้ามข้อไหนก็ได้ถ้าค่าเป็น 0):
1. ภาพรวมจำนวนงานทั้งหมด จำนวนงานที่เสร็จแล้วพร้อมเปอร์เซ็นต์ (ปัดเป็นจำนวนเต็ม) และจำนวนงานที่กำลังทำ
...
4. ถ้า overloadedMembers ไม่ว่าง ให้เอ่ยชื่อออกมาตรงๆ — ต้องคัดลอกชื่อจาก overloadedMembers มาเป๊ะๆ ทุกตัวอักษร ห้ามแต่งชื่อขึ้นเองหรือเปลี่ยนเป็นชื่ออื่นเด็ดขาด ถ้า overloadedMembers ว่างเปล่าให้ข้ามข้อนี้ไปเลย ห้ามเอ่ยชื่อคนที่ไม่มีในลิสต์นี้
5. ปิดท้ายด้วยคำแนะนำเชิงปฏิบัติ 1 ประโยคที่ผู้จัดการทำตามได้ทันที (เช่น ควรติดตามใคร หรืองานไหนก่อน) — ถ้าเอ่ยชื่อคนในคำแนะนำ ต้องเป็นชื่อจาก overloadedMembers เท่านั้น`
          ...
          contents: [
            {
              role: 'user',
              parts: [{
                text: `${systemPrompt}\n\nข้อมูลสถิติภาระงาน:\n${JSON.stringify(stats)}\n\nรายชื่อสมาชิกที่ภาระงานเกินปกติ (overloadedMembers) — ใช้ชื่อจากตรงนี้เป๊ะๆ เท่านั้น ถ้าว่างเปล่าแปลว่าไม่มีใครภาระงานเกินปกติ ห้ามเอ่ยชื่อใครทั้งสิ้น: ${
                  stats?.overloadedMembers?.length ? stats.overloadedMembers.join(', ') : '(ว่างเปล่า — ไม่มี)'
                }`,
              }]
            }
          ],
